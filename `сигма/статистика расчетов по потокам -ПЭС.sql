select cs.account_per_id,
       ...
       round(cs.actual_point_calculated /
             (((cs.calc_session_date_e - cs.calc_session_date_b) * 24 * 3600) +
             0.00001),
             2) speed,
       cs.*
  from calc_session cs, c_terra ct
 where ct.terra_id = cs.terra_id
   and cs.calc_type = 1
   and trunc(cs.calc_session_date_b) >= to_date('27.02.2019', 'DD.MM.YYYY')
   and ct.terra_id in (
                       
                       select pc.terra_id
                         from prior_calc pc, c_terra ct, org_unit ou
                        where pc.terra_id = ct.terra_id
                          and ct.terra_org_unit_id = ou.org_unit_id
                          and ou.company_id = 2202)
